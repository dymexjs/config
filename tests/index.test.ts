...
describe("@Dymexjs/config", () => {
    ...
    describe("getSection", () => {
        ...
        test("get required section success", async () => {
            const dict = { mem1: { keyinmem1: "ValueInMem1" }, mem2: { keyinmem2: { deep1: "ValueDeep1" } } };
            const configurationBuilder = new ConfigurationBuilder();
    ...
    describe("Configuration", () => {
        ...
        test("should set complex property with complex key and complex value", () => {
            ...
            assert.deepEqual(config.configuration, { key: { key1: { key2: { key3: "value" } } } });
        });
        test("should replace old values", () => {
            ...
            assert.deepEqual(config.configuration, { key: { key1: { key2: { key3: "value2" } } } });
        });
        test("should join values into existing key", () => {
            const config = new Configuration();
            config.set(`key.key1`, { [`key2.key3`]: "value1" });
            config.set(`key.key1.key2.value2`, "value2");
            assert.deepEqual(config.get(`key.key1`), { key2: { key3: "value1", value2: "value2" } });
            assert.deepEqual(config.configuration, { key: { key1: { key2: { key3: "value1", value2: "value2" } } } });
        });
    ...
    describe("multiple sources", () => {
        test("all sources simple", async (t) => {
            ...
            t.mock.method(Array.from(configBuilder.sources.values())[2], "readFile" as any, async () =>
                JSON.stringify({ JSON: "test" }),
            );
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            t.mock.method(Array.from(configBuilder.sources.values())[3], "readFile" as any, async () => ({
                JS: "test",
            }));
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            t.mock.method(Array.from(configBuilder.sources.values())[4], "readFile" as any, async () =>
                JSON.stringify({ USER_SECRETS: "test" }),
            );
            t.mock.method(
